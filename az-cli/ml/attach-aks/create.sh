...
## Get AKS Cluster ID
echo "Get AKS Cluster ID"
ML_AKS_ID=$(az aks show -g $RG_NAME -n $CLUSTER_NAME --query id -o tsv)
echo "AKS ID: $ML_AKS_ID"
echo ""

## Attach AKS to ML
echo "Attach AKS to ML workspace"
az ml computetarget attach aks \
  --compute-resource-id $ML_AKS_ID \
  --name $CLUSTER_NAME \
  --resource-group $ML_RG_NAME \
  --workspace-name $ML_WORK_SPACE_NAME \
  --debug
